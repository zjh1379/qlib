...
- **✅ Step-0 实测结果(2026-06-11,Claude 直接内联跑通)**:
  - 新闻 `stock_news_em(symbol='600519')` → 列 `['关键词','新闻标题','新闻内容','发布时间','文章来源','新闻链接']`,10 条近期新闻可用 → 采用 `新闻标题`/`发布时间`/`文章来源`。
  - 公告改用**按个股**接口 `stock_zh_a_disclosure_report_cninfo(symbol, market='沪深京', start_date, end_date)`(巨潮)→ 列 `['代码','简称','公告标题','公告时间','公告链接']`,贵州茅台近 5 周 12 条真实公告 → `fetch_notices` 做成真的(`公告标题`/`公告时间`,约 60 天窗口),**非 stub**。
  - Claude:`messages.parse` 在 `anthropic 0.109.1` 中存在(hasattr 确认);实时端到端解析放到 Task 13 冒烟(运行环境无 `ANTHROPIC_API_KEY`)。
  - **门禁结论:通过** —— 新闻 + 公告均按个股可得,风险旗标可用官方公告(信号优于纯新闻)。
